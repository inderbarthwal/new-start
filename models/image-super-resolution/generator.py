...
class Generator(nn.Module):
    def __init__(self, num_rrdb_blocks=32):
        ...
        for _ in range(num_rrdb_blocks):
            rrdb_blocks += [ResidualInResidualDenseBlock(in_channels=64, growth_channels=32, scale_ratio=0.1)]
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        out = F.leaky_relu(self.up1(F.interpolate(out, scale_factor=2, mode="nearest")), 0.1, True)
        out = F.leaky_relu(self.up2(F.interpolate(out, scale_factor=2, mode="nearest")), 0.1, True)
        out = self.conv3(out)
        out = self.conv4(out)
...
class ResidualDenseBlock(nn.Module):
    ...
    def __init__(self, in_channels: int = 64, growth_channels: int = 32, scale_ratio: float = 0.1):
...
def _super_resolution(arch, num_residual_block, pretrained, progress):
